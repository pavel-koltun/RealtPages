...
/**
 * Performance test for the ApartmentRent entity.
 */
class ApartmentRentGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the ApartmentRent entity")
        ...
        .repeat(2) {
            ...
            .headers(headers_http_authenticated)
            .body(StringBody("""{"id":null, "type":null, "isOwner":null}""")).asJSON
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_apartmentRent_url")))
            .pause(10)
            .repeat(5) {
                exec(http("Get created apartmentRent")
                .get("${new_apartmentRent_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created apartmentRent")
            .delete("${new_apartmentRent_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
